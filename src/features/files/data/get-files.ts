...
import { getFilesSchema, GetFilesSchema } from './get-files-schema';
...
export async function getFiles(input: GetFilesSchema): Promise<{
  files: FileListDTO[];
  totalCount: number;
}> {
  const result = getFilesSchema.safeParse(input);
  if (!result.success) {
